...
if len(picks) == 1:
    ...
    header = f"{formatted_date} Underdog ML Picks:"
    tweet_text = f"{header}\n\n" + "⚾ " + "\n⚾ ".join(picks) + "\n\n" + "Few picks today from our model, hopefully we get a couple hits! Let's see if we can sweep this slate! 🧹🧹 RT if you're riding!🔁" + "\n\n" + " ".join(random.sample(hashtags, 2))
elif len(picks) >= 5 and len(picks) <7 :
    header = f"{formatted_date} Underdog ML Picks:"
    tweet_text = f"{header}\n\n" + "⚾ " + "\n⚾ ".join(picks) + "\n\n" + f"Our model found {len(picks)} underdogs with good value today! Got a lot on the line here, let's see what we can do!💰💰RT if you're riding!🔁" + "\n\n" + " ".join(random.sample(hashtags, 2))
elif len(picks) >= 7:
    header = f"{formatted_date} Underdog ML Picks:"
    tweet_text = f"{header}\n\n" + "⚾ " + "\n⚾ ".join(picks) + "\n\n" + "Our model LOVES todays slate of dogs! We have a ton on the line today, let's see how we do!💰💰 RT if you're riding!🔁" + "\n\n" + " ".join(random.sample(hashtags, 2))
else:
